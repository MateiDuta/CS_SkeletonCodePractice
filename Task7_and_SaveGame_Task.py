...
class Breakthrough():
    ...
    def __PlayCardToSequence(self, CardChoice):
        ...
        if self.__Hand.GetCardDescriptionAt(CardChoice - 1)[2] == "m":
            inp = input("What Toolkit do you want to assign the card to (a), (b), (c)?")
            if inp == "a":
                self.__Hand._Cards[CardChoice - 1].SetCardToolkit("a")
            elif inp == "b":
                self.__Hand._Cards[CardChoice - 1].SetCardToolkit("b")
            elif inp == "c":
                self.__Hand._Cards[CardChoice - 1].SetCardToolkit("c")
        # end change
...
class Lock():
    ...
    def GetLockSolvedStatus(self):
        ...
        for i in self._Challenges:
            ...
            MetList.append(";")
        MetList.pop()
        MetList = "".join(MetList)
        return MetList

    def GetLockString(self):
        LockList = []
        for i in self._Challenges:
            LockList.append(",".join(i.GetCondition()))
            LockList.append(";")
        LockList.pop()
        LockList = "".join(LockList)
        return LockList
