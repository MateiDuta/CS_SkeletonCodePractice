        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    elif MenuChoice == "M" and not self.MulliganUsed:
                        self.MulliganUsed = True
                        for i in range(5):
                            self.__MoveCard(self.__Hand, self.__Deck, self.__Hand.GetCardNumberAt(i))

                        for i in range(self.__Discard.GetNumberOfCards()):
                            self.__MoveCard(self.__Discard, self.__Deck, self.__Discard.GetCardNumberAt(i))

                        for i in range(self.__Sequence.GetNumberOfCards()):
                            self.__MoveCard(self.__Sequence, self.__Deck, self.__Sequence.GetCardNumberAt(i))

                        self.__Deck.Shuffle()

                        for i in range(5):
                            if self.__Deck.GetCardDescriptionAt(i) == " Dif":
